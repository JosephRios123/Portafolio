...
const empty: Omit<Experience, "id"> & { bullets: Bullet[] } = {
  role: "", company: "", start_date: "", end_date: "", is_current: false, color: "hsl(217 91% 60%)", display_order: 0, bullets: [],
};

export default function ExperienceAdmin() {
  const [items, setItems] = useState<Experience[]>([]);
  const [loading, setLoading] = useState(true);
  const [editing, setEditing] = useState<(Omit<Experience, "id"> & { id?: string; bullets: Bullet[] }) | null>(null);
  const [saving, setSaving] = useState(false);

  const load = async () => {
    setLoading(true);
    const { data, error } = await supabase.from("experiences").select("*").order("display_order");
    if (error) toast.error(error.message); else setItems((data ?? []) as Experience[]);
    setLoading(false);
  };
  useEffect(() => { load(); }, []);
  ...
  const openEdit = async (exp: Experience) => {
    ...
    setEditing({ ...exp, bullets: (data ?? []).map((b) => ({ id: b.id, text: b.text, display_order: b.display_order })) });
  };

  const save = async () => {
    if (!editing) return;
    if (!editing.role.trim() || !editing.company.trim() || !editing.start_date.trim()) {
      toast.error("Cargo, empresa y fecha de inicio son obligatorios"); return;
    }
    setSaving(true);
    const payload = {
      role: editing.role, company: editing.company, start_date: editing.start_date,
      end_date: editing.is_current ? null : (editing.end_date || null),
      is_current: editing.is_current, color: editing.color, display_order: editing.display_order,
    };
    ...
    if (editing.bullets.length) {
      const rows = editing.bullets.filter((b) => b.text.trim()).map((b, i) => ({ experience_id: expId, text: b.text, display_order: i }));
      if (rows.length) await supabase.from("experience_bullets").insert(rows);
    }
    toast.success("Guardado"); setEditing(null); load(); setSaving(false);
  };
  ...
  return (
    <div>
      ...
      {loading ? <div className="flex justify-center py-12"><Loader2 className="animate-spin text-accent" /></div>
        : items.length === 0 ? <div className="glass-card rounded-2xl p-12 text-center text-muted-foreground">Sin experiencias.</div>
        : (
          <div className="grid gap-4">
            {items.map((e) => (
              <div key={e.id} className="glass-card rounded-2xl p-5 flex items-center gap-4">
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={() => setEditing(null)}>
          <div className="bg-card border border-border rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6 sm:p-8" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              <Field label="Cargo"><input value={editing.role} onChange={(e) => setEditing({ ...editing, role: e.target.value })} className="input-base" /></Field>
              <Field label="Empresa"><input value={editing.company} onChange={(e) => setEditing({ ...editing, company: e.target.value })} className="input-base" /></Field>
              <div className="grid grid-cols-2 gap-3">
                <Field label="Fecha inicio"><input value={editing.start_date} onChange={(e) => setEditing({ ...editing, start_date: e.target.value })} placeholder="Ene 2023" className="input-base" /></Field>
                  ...
                  {editing.bullets.map((b, i) => (
                    <div key={i} className="flex gap-2">
                      <input value={b.text} onChange={(e) => {
                        const bullets = [...editing.bullets]; bullets[i] = { ...b, text: e.target.value };
